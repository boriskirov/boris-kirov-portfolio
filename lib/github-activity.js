...
function getCommitRepositoryEntries(collection) {
  return (collection.commitContributionsByRepository ?? [])
    ...
    .sort(
      (a, b) => new Date(b.latestOccurredAt) - new Date(a.latestOccurredAt),
    );
}
    ...
    commits
      ...
      .map(async (commit) => {
        ...
        return {
          ...
          url:
            commit.html_url ||
            `https://github.com/${nameWithOwner}/commit/${commit.sha}`,
...
async function fetchPullRequestItems(token, since) {
  ...
  if (!response.ok) {
    throw new Error(
      `GitHub pull request search failed with ${response.status}`,
    );
  }
...
async function fetchGitHubPublicActivity() {
  ...
  if (!response.ok) {
    throw new Error(
      `GitHub public events request failed with ${response.status}`,
    );
  }
